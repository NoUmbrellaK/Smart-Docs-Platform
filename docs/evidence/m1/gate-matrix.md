...
| F-01.3 | M1: move only within one project; keep file/version IDs stable | Direct | `file_mutation_enforces_roles_project_scope_conflicts_and_stable_ids` in [file_mutation_test.cpp](../../../test/integration/file_mutation_test.cpp) (`:153-188`) and HTTP T-08 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:467-484`); cross-project movement is rejected, not implemented | Current evidence pending |
...
| F-01.5a | M1: web soft-delete/restore; hide immediately from the default list; do not auto-purge | Mixed: direct for delete/restore/default-list hiding; indirect/missing for retention and no auto-purge | HTTP T-08 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:485-497`) and delete/restore controls in [app.html](../../../resources/app.html) (`:90-99`) cover the interactive behavior; no named test asserts retention over time or absence of automatic purge | Current evidence pending; retention/no-auto-purge direct proof missing |
...
| F-02.4 | M1: same task/number/content is reused; different content conflicts | Direct | HTTP T-02 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:319-336`) and replay/conflict integration coverage in [upload_part_test.cpp](../../../test/integration/upload_part_test.cpp) (`:280-300`) | Current evidence pending |
| F-02.5 | M1: acknowledge a part only after durable completion; never confirm a half-written part | Direct | `AfterPartTempFsync` recovery assertions in [m1_interrupt_test.py](../../../test/e2e/m1_interrupt_test.py) (`:19-25`, `:297-324`, `:358-365`) and unfinished-writer cleanup in [file_store_test.cpp](../../../test/unit/file_store_test.cpp) (`:137-153`) | Current evidence pending; the visible 20-round artifact is from an older commit |
...
| F-02.7 | M1: completion is idempotent and returns stable file/version/job IDs | Direct | HTTP T-04 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:367-386`), idempotent/zero-byte and concurrent-single-graph cases in [upload_complete_test.cpp](../../../test/integration/upload_complete_test.cpp) (`:204-287`) | Current evidence pending |
| F-02.8 | M1: interrupted completion exposes no partial object; restart reconciles records and disk | Direct | Six fault points and 20 rounds in [m1_interrupt_test.py](../../../test/e2e/m1_interrupt_test.py) (`:19-25`, `:312-387`, `:433-449`); the harness leak detector in [m1_harness_self_test.py](../../../test/e2e/m1_harness_self_test.py) (`:199-303`) | Current evidence pending; the visible 20-round artifact is from an older commit |
...
| ID | Requirement clause and stage | Design coverage | Concrete test or evidence | Run evidence |
| --- | --- | --- | --- | --- |
| F-03.1 | M1: authorize ordinary and Range downloads; no public-static bypass | Direct | HTTP T-01 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:301-317`) and shell asset allowlist in [webserver_socket_test.cpp](../../../test/unit/webserver_socket_test.cpp) (`:134-166`) | Current evidence pending |
| F-03.2 | M1: support closed, open-ended, and suffix single ranges with correct range/total length | Direct | HTTP T-05 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:388-406`) and `range_parses_closed_open_and_suffix_forms_with_clamping` in [range_test.cpp](../../../test/unit/range_test.cpp) (`:34-45`) | Current evidence pending |
| F-03.3 | M1: return an explicit error for invalid ranges and do not fake multi-range success | Direct | HTTP T-05 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:407-417`) and response coverage in [file_download_test.cpp](../../../test/integration/file_download_test.cpp) (`:301-312`) | Current evidence pending |
| F-03.4 | M1: bind a transfer/preview to one immutable version | Direct | `file_download_current_binds_one_immutable_version_and_history_is_exact` in [file_download_test.cpp](../../../test/integration/file_download_test.cpp) (`:153-193`) and HTTP T-06 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:427-450`) | Current evidence pending |
| F-03.5 | M1: a real PDF component renders and navigates to a requested page; Range support alone is insufficient | Indirect; browser proof missing | The iframe/page controls in [app.html](../../../resources/app.html) (`:100-110`), URL fragment assignment in [app.js](../../../resources/js/app.js) (`:720-730`), and HTTP T-05 bytes/header/fragment checks in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:388-425`) do not demonstrate actual browser rendering or page navigation | Current evidence pending; real browser PDF render/page-flip proof missing |
...
| ID | Requirement clause and stage | Design coverage | Concrete test or evidence | Run evidence |
| --- | --- | --- | --- | --- |
| F-04.1 | M1: never overwrite an original version; advance current; keep old versions downloadable | Direct | HTTP T-06 and T-07 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:427-465`) and immutable-version coverage in [file_download_test.cpp](../../../test/integration/file_download_test.cpp) (`:153-193`) | Current evidence pending |
...
| F-04.3 | M2/M3: historical citations open the original version and never silently jump to a newer one | Deferred M2+ | HTTP T-06 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:427-450`) proves the exact-version URL prerequisite only; M1 has no citation object or citation-click flow | Not an M1 gate numerator |
| F-04.4a | M1: delete, restore, move, and permission changes affect subsequent file access checks | Direct | HTTP T-08 in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:467-515`) and deleted historical-content rejection in [file_download_test.cpp](../../../test/integration/file_download_test.cpp) (`:196-226`) | Current evidence pending |
...
| --- | --- | --- | --- | --- |
| T-01 | Project/role restrictions and ID substitution: HTTP `T-01` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:301-317`) directly rejects unauthorized list, download, and Range requests; [auth_project_test.cpp](../../../test/integration/auth_project_test.cpp) (`:164-237`) covers project permissions; [file_metadata_test.cpp](../../../test/integration/file_metadata_test.cpp) (`:269-275`) rejects unauthorized exact-version opening at the service layer | Direct for list/download/Range and ID substitution; indirect for service-level preview authorization; preview-specific HTTP/browser negative proof missing | Search, tools, and reports are M2-M4 | Current evidence pending; preview authorization proof missing |
| T-02 | Replayed identical chunk is reused; different content conflicts: HTTP `T-02` (`:319-336`) in [m1_http_test.py](../../../test/e2e/m1_http_test.py) | Direct | None | Current evidence pending |
| T-03 | Server restart, true confirmed-parts state, missing-part upload, final hash, and half-part retry: HTTP `T-03` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:338-365`) plus `AfterPartTempFsync` in [m1_interrupt_test.py](../../../test/e2e/m1_interrupt_test.py) (`:297-324`, `:358-377`) | Direct for server restart; indirect/missing for browser refresh and reselect | None; browser refresh is an M1 obligation, not deferred | Current evidence pending; real browser refresh-resume-reselect proof missing |
| T-04 | Repeated/lost completion returns the same IDs and hides partial content: HTTP `T-04` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:367-386`) plus commit/response fault points in [m1_interrupt_test.py](../../../test/e2e/m1_interrupt_test.py) (`:318-387`) | Direct | “Not searchable” is M2; M1 proves list/download invisibility only | Current evidence pending |
| T-05 | Closed/open/suffix and invalid Range behavior: HTTP `T-05` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:388-417`) | Direct for Range; indirect/missing for PDF rendering/page navigation | None; real PDF UI is an M1 obligation | Current evidence pending; real browser PDF render/page-flip proof missing |
| T-06 | Exact historical/current version URLs and immutable transfer: HTTP `T-06` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:427-450`) plus [file_download_test.cpp](../../../test/integration/file_download_test.cpp) (`:153-193`) | Direct for the M1 version prerequisite | Default search and citation behavior are M2/M3 | Current evidence pending |
| T-07 | M1 precursor: an older upload cannot replace current, HTTP `T-07` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:452-465`) | Direct precursor only | The specified stale parsing/default-index scenario is M2 | Current evidence pending for the precursor; deferred clause is not an M1 numerator |
| T-08 | List/download follow move, delete, restore, and membership changes: HTTP `T-08` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:467-515`) | Direct for M1 list/download state | Search/cache are M2, tools are M3, and reports/implicit report context are M4 | Current evidence pending |
| T-09 | M1 exposes pending/processing/failed status and does not claim parsing success: HTTP `T-09` in [m1_http_test.py](../../../test/e2e/m1_http_test.py) (`:517-532`) | Direct precursor only | Scanned/damaged/oversized parsing and keyword/vector fallback are M2 | Current evidence pending for the precursor; deferred clause is not an M1 numerator |

## Current closure blockers

| # | Status | Blocker and evidence |
| --- | --- | --- |
| 1 | Missing | No accepted full gate artifact exists for the checkout being documented. The only visible `docs/evidence/m1/latest/results.json` is bound to `a33fb99715ddea0c657f61ca5a2914baca2cb6d8`; the implementation baseline inspected before this uncommitted documentation change was `3762c6bfb939a6cbf31fd4287ab95532c7a68085`. `latest/environment.txt`, `latest/summary.md`, and all reviewed `verified/` files are absent. |
...
| 5 | Missing | The current [stage runner](../../../test/e2e/run_m1.sh) builds `server`, `admin`, and `test-server` and records the Python HTTP/fault scenarios, but it does not run or aggregate ordinary C++ unit/integration suite counts or UI contract/behavior suite counts. |
...
| 8 | Missing | T-01 has service-layer denial for unauthorized exact-version opening, but no preview-specific negative HTTP/browser scenario. Preview authorization therefore remains indirect rather than a direct T-01 pass. |
